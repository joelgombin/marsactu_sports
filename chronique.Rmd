...
Peut-être toutefois que comparer toutes les communes entre elles n'a pas de sens. Leur taille est très différente, leur composition (par âge, par sexe, par catégorie socio-professionnel...). Qu'en est-il alors si on cherche à contrôler ces facteurs, et à regarder ce qu'il en est de la pratique sportivce (licenciée) à Marseille "toutes choses égales par ailleurs", comme disent les économistes ? 

```{r modele}

load("/media/Data/Dropbox/Thèse/données propres/RP2012/RP2012_POP6_comm.Rdata")

RP2012_CS <- RP2012_POP6_comm %>% 
  filter(!is.na(CODGEO)) %>% 
  group_by(CODGEO, CS1_8) %>% 
  summarise(NB = sum(NB)) %>% 
  spread(CS1_8, NB)

RP2012_AGE <- RP2012_POP6_comm %>% 
  filter(!is.na(CODGEO)) %>% 
  group_by(CODGEO, AGEQ65) %>% 
  summarise(NB = sum(NB)) %>% 
  spread(AGEQ65, NB)

RP2012_SEXE <- RP2012_POP6_comm %>% 
  filter(!is.na(CODGEO)) %>% 
...
df <- licences %>% 
  ...
  gather(variable, valeur, fed_2012:poph_75_99_2010) %>% 
  left_join(variables %>% select(Nom_Colonne, Legende), by = c("variable" = "Nom_Colonne")) %>% 
  filter(variable == "l_2012") %>% 
  select(cog2, Federation, valeur) %>% 
  mutate(valeur = as.integer(valeur)) %>% 
  complete(cog2, Federation, fill = list(valeur = 0)) %>% 
  left_join(RP2012_CS, c("cog2" = "CODGEO")) %>% 
  left_join(RP2012_AGE, c("cog2" = "CODGEO")) %>% 
  left_join(RP2012_SEXE, c("cog2" = "CODGEO")) %>% 
  mutate(habitants = femmes + hommes)
modele_general <- glmer(valeur ~ `agriculteurs exploitants` +  `artisans, commerçants, chefs entreprise` + `autres personnes sans activité professionnelle` + `cadres et professions intellectuelles supérieures`+ `employés` + `ouvriers` + `professions intermédiaires` +  `retraités` + `15 à 19 ans` + `20 à 24 ans` + `25 à 29 ans` + `30 à 34 ans` + `35 à 39 ans` + `40 à 44 ans` + `45 à 49 ans` + `50 à 54 ans` + `55 à 59 ans` + `60 à 64 ans` + `65 ans ou plus` + `femmes` + `hommes` + (1 + `agriculteurs exploitants` + `artisans, commerçants, chefs entreprise` + `autres personnes sans activité professionnelle` + `cadres et professions intellectuelles supérieures`+ `employés` + `ouvriers` + `professions intermédiaires` +  `retraités` + `15 à 19 ans` + `20 à 24 ans` + `25 à 29 ans` + `30 à 34 ans` + `35 à 39 ans` + `40 à 44 ans` + `45 à 49 ans` + `50 à 54 ans` + `55 à 59 ans` + `60 à 64 ans` + `65 ans ou plus` + `femmes` + `hommes` | Federation), data = df, family = poisson(link = "identity"))
